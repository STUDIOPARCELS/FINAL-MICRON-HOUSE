...
const useCases = [
  ...
  // 2. Green -> Deep Eggplant Hover (Better Contrast than Light Eggplant)
  ...
  // 3. Light Eggplant -> Green Hover (New Color Added)
  ...
  // 6. Grey 1 -> Green Hover (Reduced Grays)
  ...
  // 7. Light Eggplant -> Green Hover (New Color Added)
  ...
  // 9. Green -> Deep Eggplant Hover
  { id: 9, title: "Pre-Event Cocktail Parties", text: "Pre-Treefort cocktails. Post-Philharmonic gathering.", icon: <Music />, gradient: "bg-micron-green", hoverClass: "group-hover:text-micron-eggplant" },
  
  // 10. Deep Eggplant -> Green Hover
  ...
  return (
    <section id="use-cases" className="container mx-auto px-6 py-24 md:px-12 bg-white text-zinc-900">
      <div className="mb-16 flex flex-col md:flex-row md:items-end gap-6 border-b border-zinc-200 pb-8">
        <div>
           <span className="block text-xs font-bold uppercase tracking-[0.2em] text-zinc-400 mb-2">02 / USE CASE</span>
           <h2 className="text-5xl md:text-6xl font-bold uppercase tracking-tight text-zinc-900 leading-none">EXPERIENCES</h2>
        </div>
        ...
        {useCases.map((uc, i) => (
          ...
          >
            <div className="flex justify-between items-start mb-4">
                ...
                <span className="text-[10px] font-bold text-white/30 group-hover:text-white/60 transition-colors">0{i+1}</span>
            </div>
            
            <div>
               <h3 className={`font-bold text-sm text-white mb-2 leading-tight ${uc.hoverClass} transition-colors duration-300`}>{uc.title}</h3>
               <p className="text-[10px] leading-relaxed font-body text-white/50 line-clamp-3 group-hover:text-white/80 transition-colors duration-300">{uc.text}</p>
            </div>
